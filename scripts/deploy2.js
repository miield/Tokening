...
async function main() {
  ...
  const tkn = await deployedContract.connect(signer2).createToken(signer2.address, "Oyindamola", "OYT", 250000);
  ...
  const vToken = await deployedContract.connect(signer2).viewToken();
